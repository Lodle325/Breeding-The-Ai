...
// ============================================================
var CTRL_POINTS = [
  { x: 400, y: 150 }, // 0: start / finish  ← middle of top straight
  { x: 145, y: 115 }, // 1: hairpin approach
  { x: 200, y: 250 }, // 2: hairpin apex
  { x: 98, y: 340 }, // 3: hairpin exit
  { x: 205, y: 392 }, // 4: bottom left turn
  { x: 350, y: 300 }, // 5: bottom straight
  { x: 470, y: 402 }, // 6: chicane entry
  { x: 562, y: 372 }, // 7: chicane right
  { x: 518, y: 308 }, // 8: chicane left
  { x: 608, y: 268 }, // 9: final corner
  { x: 620, y: 120 }, // 10: top right kink
];
...
var TURN_RATE = 0.055; // tighter cornering
var MAX_GRASS_FRAMES = 160;
var MAX_STUCK_FRAMES = 220;

// Lerp rates for smooth acceleration / deceleration
var ACCEL_RATE = 0.068; // how quickly speed approaches max when accelerating
...
Car.prototype.update = function (controls) {
  ...
  var turnSf = Math.min(1.0, Math.abs(this.speed) / (MAX_SPEED * 0.3));
  if (turnSf > 0.05) {
  ...
  if (onGrass) {
    this.grassFrames++;
    if (this.grassFrames > MAX_GRASS_FRAMES) {
      this.alive = false;
      return;
    }
  } else {
  ...
  if (Math.abs(this.speed) < 0.22 && this.timeAlive > 100) {
    this.stuckFrames++;
    if (this.stuckFrames > MAX_STUCK_FRAMES) {
      this.alive = false;
      return;
    }
  } else {
